package com.aaliyah.sakila.manager;

import java.util.ArrayList;
import java.util.List;

import com.aaliyah.sakila.model.Film;

public class FilmManager implements CrudManager<Film> {

    private final ArrayList<Film> items = new ArrayList<>();

    @Override
    public void post(Film obj) {
        if (obj.getId() == 0) {
            int next = items.stream().mapToInt(Film::getId).max().orElse(0) + 1;
            obj.setId(next);
        }
        items.add(obj);
    }

    @Override
    public Film get(int id) {
        return items.stream().filter(i -> i.getId() == id && i.isActive()).findFirst().orElse(null);
    }

    @Override
    public List<Film> get(String filter) {
        List<Film> res = new ArrayList<>();
        for (Film i: items) {
            if (!i.isActive()) continue;
            String s = i.toString();
            if (s.toLowerCase().contains(filter.toLowerCase())) res.add(i);
        }
        return res;
    }

    @Override
    public void put(Film obj) {
        Film existing = get(obj.getId());
        if (existing != null) {
            
            int idx = items.indexOf(existing);
            items.set(idx, obj);
        }
    }

    @Override
    public void delete(int id) {
        Film e = get(id);
        if (e != null) e.setActive(false); 
    }

    public List<Film> all() {
        List<Film> out = new ArrayList<>();
        for (Film i: items) if (i.isActive()) out.add(i);
        return out;
    }
}
